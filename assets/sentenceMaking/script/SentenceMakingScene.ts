...
export class SentenceMakingScene extends BaseScene<IBaseGameChild> {
    ...
    private async startGameFlow() {
        ...
        if ((this.sceneModel as any).gameType == GameType.SKEWERS) {
            ...
            let model = (this.sceneModel as any);
            let level = model.game.result[model.difficulty-1].levels.indexOf(model.game.getLevelByDifficult(model.difficulty)) + 1;
            if(level<=0){
                level = 1;
            }
            if (!Global.isAgain) {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            for (let [key, inst] of this.sourceContainerMap) {
                tween(inst).delay(this.currentQuestion.sentence.length * moveDuration + key * flipDelay).call(() => {
                    ...
                    this.playAudio("audio/majiang", true);
