...
export class TaskItemController extends Component {
    ...
    async setTaskBg(spritePath) {
        ...
        this.btnBg.spriteFrame = spriteFrame;

        // 设置宽高，考虑适配缩放系数
        // if (width !== undefined || height !== undefined) {
        //     const uiTransform = this.btnBg.node.getComponent(UITransform);
        //     if (uiTransform) {
    ...
    async setbgColor(bg0_color: string, bg1_color: string) {
        // 自动检测颜色字符串格式并解析
        ...
        //     if (this.btnBg) {
        //         this.btnBg.color = color3;
        //         DebugLog.instance.log(`btnBg 颜色已设置: ${this.btnBg.color}`);
        //     } else {
        //         DebugLog.instance.warn("btnBg 组件不存在");
        //     }
        // }
    }
    ...
    async setTaskIcon(spritePath, colorStr) {
